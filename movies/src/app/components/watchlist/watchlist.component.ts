...
export class WatchlistComponent implements OnInit {
  ...
  remove(movie: any) {
    this.dataService.post('/user/remove', {id: movie.id}).subscribe(res => {
      ...
      this.dataService.get('/user').subscribe(result => { // fetch updated movielsit
        this.moviesList = result.user.data.movielist;
        this.eventService.publish(result.user.data.movielist.length); // publish changes (movielist length)
      });
    });
  }
